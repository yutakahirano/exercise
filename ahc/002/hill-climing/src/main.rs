...
fn optimize(map: Map, src: usize, dest: usize, full_best: &State) -> State {
    let mut map = map;
    assert!(src < dest);
    ...
    let history = full_best.history[src..(dest + 1)].to_vec();
    let score = history
        .iter()
        .map(|p| map.reward_dict.get(p).unwrap())
        .sum();
    let mut best = State { score, history };
    let mut stack = vec![StateOrRollback::State(State {
        score: 0,
        history: vec![best.history[0].clone()],
    })];

    let mut counter = 0_u32;
    let max_counter = 2000;
...
fn hillclimb(map: &mut Map, starting_position: &Position) -> Option<State> {
    let new_map = map.clone();
    let mut best = dfs(map, starting_position)?;
    let mut rng = rand::thread_rng();
    let max_counter = 2 * 1000;
    for _ in 0..max_counter {
        let length = rng.gen_range(3..50);
        let src = rng.gen_range(0..(best.history.len() - length));
        let dest = src + length - 1;
        best = optimize(new_map.clone(), src, dest, &best);
    }
...
fn main() {
    ...
    let best = hillclimb(&mut map, &starting_position).unwrap();
    print_history(&best);
}
