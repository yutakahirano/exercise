...
impl Position {
    fn distance(x: &Position, y: &Position) -> usize {
        let dx = x.x.abs_diff(y.x);
        let dy = x.y.abs_diff(y.y);
        dx + dy
    }
}
...
fn optimize(map: Map, src: usize, dest: usize, full_best: &State, temperature: f64) -> State {
    let mut map = map;
    assert!(src < dest);
    let dest_pos = &full_best.history[dest];

    for i in 0..full_best.history.len() {
        if i < src || i > dest {
            map.visit(&full_best.history[i]);
        }
    }
    let mut best: Option<State> = None;
    let mut stack = vec![StateOrRollback::State(State {
        score: 0,
        history: vec![full_best.history[src].clone()],
    })];

    let mut rng = rand::thread_rng();
    let mut counter = 0_u32;
    let max_counter = 100;
    let max_length = std::cmp::min(1, dest - src) * 20;
    ...
    while let Some(s_or_r) = stack.pop() {
        match s_or_r {
            StateOrRollback::State(state) => {
                ...
                if Position::distance(&position, dest_pos) > max_length {
                    continue;
                }
                ...
                let mut candidates = map.next_candidates(&position);
    ...
    if best.is_none() {
        return full_best.clone();
    }
    let best = best.unwrap();

    let mut new_history = full_best.history[..src].to_vec();
    new_history.extend(best.history.iter().cloned());
    new_history.extend(full_best.history[(dest + 1)..].iter().cloned());
    let new_score = new_history
        .iter()
        .map(|p| map.reward_dict.get(p).unwrap())
        .sum();
    let new_best = State {
        score: new_score,
        history: new_history,
    ...
    let target_score: u32 = full_best.history[src..(dest + 1)]
        .iter()
        .map(|p| map.reward_dict.get(p).unwrap())
        .sum();
    ...
    let diff = (target_score - best.score) as f64;
    if (-diff / (temperature + 1e-7)).exp() > rng.gen_range(0.0..1.0) {
        return new_best;
    }
...
fn simulated_annealing(map: &mut Map, starting_position: &Position) -> Option<State> {
    let new_map = map.clone();
    let mut best = dfs(map, starting_position)?;
    let mut rng = rand::thread_rng();
    ...
    loop {
        ...
        let temperature = 500.0 * (1.0 - (now - start).as_secs_f64() / duration.as_secs_f64());
        let min_cut_length = 20_usize;
        let max_cut_length = std::cmp::max(
            min_cut_length + 1,
            (0.03 * best.history.len() as f64) as usize,
        );
        let length = std::cmp::min(
            best.history.len(),
            rng.gen_range(min_cut_length..max_cut_length),
        );
        let src = rng.gen_range(0..(best.history.len() - length));
        let dest = src + length - 1;
        best = optimize(new_map.clone(), src, dest, &best, temperature);
    }
...
fn main() {
    ...
    let best = simulated_annealing(&mut map, &starting_position).unwrap();
    print_history(&best);
